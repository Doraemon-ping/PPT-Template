...
window.NamedProjects=(function(){
  ...
  async function request(path,options){const r=await fetch(path,options);if(!r.ok){const text=await r.text().catch(()=> '');const rid=r.headers.get('X-Request-Id')||'';console.error('[DFM] 接口错误',path,r.status,rid,text);throw new Error(dfmReadError(r.status,text,rid));}return r.json();}
